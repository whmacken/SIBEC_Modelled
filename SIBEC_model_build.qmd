...
# Step 1: Calculate non-zero SI50 count per TreeSpp
valid_species <- sibec.tst %>%
  filter(SI50 > 0) %>%
  count(TreeSpp) %>%
  filter(n >= 50) %>%
  pull(TreeSpp)
...
xx <- sibec.tst %>% filter(BGC == "ESSFmcw")
```
...
##Build ensemble Model by TreeSpp with extra data and custom meta learner

Need to add in GAM model
```{r}
...
for (species in names(species_list)) {
  ...
  if (nrow(train_data) > 50) {
    ...
    model_recipe <- recipe(reformulate(predictors, response = "SI50"), data = train_data) %>%
      ...
      step_interact(terms = ~ aSMR:SNR)

    #gam_spec <- gen_additive_mod() %>%   set_engine("mgcv") %>%  set_mode("regression")
    
   # gam_wf <- workflow() %>%  add_model(gam_spec) %>%
  #add_formula(SI50 ~ s(aSMR, k = 9) + s(SNR, k = 5) + DD5 + CMD.adj + bFFP + CMI + DD18 + Eref + PAS +rSMR) 
    ...
    gamboost_res <- fit_resamples(gamboost_wf, resamples = cv_folds, 
                                  metrics = metric_set(mae, rmse, rsq), control = stack_control)
    rf_res <- fit_resamples(rf_wf, resamples = cv_folds, 
                            metrics = metric_set(mae, rmse, rsq), control = stack_control)
    svm_res <- fit_resamples(svm_wf, resamples = cv_folds, 
                             metrics = metric_set(mae, rmse, rsq), control = stack_control)
    # gam_res <- fit_resamples(gam_wf,  resamples = cv_folds,  
    #                          metrics = metric_set(mae, rmse, rsq),  control = stack_control)
...
member_preds <- predict(ensemble_stack, new_data = train_data, members = TRUE)
  
meta_data <- bind_cols(member_preds, SI50 = train_data$SI50)

library(tidymodels)

# Meta-learner spec: Boosted tree
...
meta_fit <- fit(meta_wf, data = meta_data)

# Predict with the meta-learner
# For new data:
new_base_preds <- predict(ensemble_stack, new_data = train_data, members = TRUE)

# Generate final predictions using the fitted meta-model
final_predictions <- predict(meta_fit, new_data = new_base_preds)%>%
      bind_cols(train_data %>% select(SI50)) %>%
      mutate(TreeSpp = species)

     # Save results
  #model_list[[species]] <- ensemble_stack 
    model_list[[species]] <- meta_fit
    # Save the model
    saveRDS(ensemble_stack, file = paste0("./models/metafit_ensemble_model_", species, ".rds"))
    ...
    species_predictions[[species]] <- final_predictions
...
# Save the entire model list as one object
saveRDS(model_list, file = "models/all_species_metafit_ensemble_models.rds")
...
write.xlsx(all_predictions, "Measured_vs_Predicted_SI50.xlsx")
...
r2_by_species <- all_predictions %>%
  group_by(TreeSpp) %>%
  summarise(R2 = calculate_r2(.pred, SI50))
...
model_list <- readRDS("models/all_species_ensemble_models.rds")
# Combine metrics and predictions
