...
model_files <- list.files(path = "./models/", pattern = "metafit_ensemble_model_.*\\.rds$", full.names = TRUE)
...
#  species <- sub("./models/ensemble_model_(.*)\\.rds", "\\1", file)  # Extract species name
  species <- sub("./models/metafit_ensemble_model_(.*)\\.rds", "\\1", file)  # Extract species name
  model <- readRDS(file)
...
fwrite(ensemble_predictions_df, "./outputs/metafit_ensemble_predictions_df.csv")
toc()
...
```{r}
fread("./outputs/metafit_ensemble_predictions_df.csv") -> ensemble_predictions_df
summary_ensemble_results <- ensemble_predictions_df %>%
     group_by(BGC, TreeSpp, rSMR, SNR) %>%
  mutate(
    mean_SI50 = max(pred_si50, na.rm = TRUE),
...
fwrite(summary_ensemble_results, "./outputs/summary_rSMR_results_metafit.csv", row.names = FALSE)
# explore SI by edatopic position for a specific BGC and TreeSpp
```


```{r}
fread("./outputs/summary_rSMR_results_metafit.csv") -> summary_ensemble_results

test <- summary_ensemble_results %>% filter(BGC == "CWHvm3", TreeSpp == "Ss") %>%  mutate(SNR = recode(SNR, `1` = "A", `2` = "B", `3` = "C", `4` = "D", `5` = "E") ) %>% 
...
```
## Differences across BGCs for constant rSMR and SNR ordered by DD5
```{r}
fread("./outputs/summary_rSMR_results_metafit.csv") -> summary_ensemble_results
sibec.anal.mean <- sibec.anal %>% select(BGC, DD5) %>% group_by(BGC) %>% summarize(mean_DD5 = mean(DD5, na.rm = TRUE), .groups = "drop") %>% distinct
DD5_bgc <- left_join(summary_ensemble_results, sibec.anal.mean) %>% distinct

test <- DD5_bgc %>%
  filter(rSMR == 0, SNR == 5) %>%
  group_by(TreeSpp) %>%
  arrange(mean_DD5) %>%
  mutate(BGC_faceted = factor(paste(TreeSpp, BGC), levels = paste(TreeSpp, BGC))) %>%
  ungroup()


ggplot(test, aes(x = BGC_faceted, y = mean_SI50, fill = TreeSpp)) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~TreeSpp, scales = "free_x", ncol = 3) +  # or try "free" for full flexibility
  theme_minimal() +
  labs(
    title = "mean_SI50 by BGC (rSMR 4, SNR D)",
    x = "BGC",
    y = "mean SI50"
  ) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 5)) +
  scale_x_discrete(labels = ~gsub("^.*? ", "", .))  # keeps just the BGC name clean
```
...
bgc.choose = "CWHdm3"; tree.spp.choose = "Fd"
