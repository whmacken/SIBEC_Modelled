...
sibec_master <- dbConnect(odbc::odbc(), .connection_string = "Driver={Microsoft Access Driver (*.mdb, *.accdb)}; DBQ=D:/GitHub/SIBEC_Modelled/data/SIBEC_2024_Prov_VPRO_Master.accdb;")
...
sibec2 <- sibec %>% select(PlotNumber, Zone, BGC, SNR, Latitude, Longitude, Elevation, SMR, Aspect, SlopeGradient)
site.tree2 <- sibec.tree %>% select(PlotNumber, TreeSpp, CountAge, SiteIndex, SiteIndex24) %>% as.data.table
...
bgcxspp <- sibec %>% group_by(BGC,TreeSpp) %>% dplyr::summarize(numbersites = n()) %>% filter(numbersites >=15)
...
sibec_calc <- sibec %>% group_by(TreeSpp, Zone, BGC, SMR, SNR) %>%
    dplyr::summarize(si50 = mean(SiteIndex24), numbersites = n()) %>%   filter(numbersites >=1)
