...
sibec  <- sibec %>% mutate(BGC = paste0(Zone, SubZone), SNR = toupper(NutrientRegime)) %>% as.data.table %>% distinct()
...
sibec_calc <- sibec %>% group_by(Species, BGC, MoistureRegime, NutrientRegime) %>%
    dplyr::summarize(si50 = mean(SI50), numbersites = n()) %>%   filter(numbersites >=5)
sibec_tst <- sibec_calc %>% filter(str_detect(BGC, "^SBS"))
...
sibec_calc2 <- sibec  %>% group_by(Species, BGC, MoistureRegime, NutrientRegime) %>%   
  dplyr::mutate(n = n()) %>% filter(n >=8) %>% 
  arrange(SI50) %>% slice(3:(n() - 2)) %>%   
  dplyr::summarize(si50_2 = mean(SI50), numbersites = n())
...
# bgcs <- c("CDF", "CWH", "MH")
#spp = c("Bl")
sibec_max <- sibec %>% as.data.frame %>% arrange(sibec$SI) %>% filter(Species %in% spp,  BHA >45, between (DD5,1200, 1250)) #!Zone %in% bgcs,#, ,  aSMR %in% smr_max, NutrientRegime %in% snr_max, BGC %in% subzones)# )
