...
# bgcs <- c("CDF", "CWH", "MH")
#spp = c("Bl")
sibec_max <- sibec %>% as.data.frame %>% arrange(sibec$SI) %>% filter(Species %in% spp,  BHA >15, DD5<2500) #!Zone %in% bgcs,#, ,  aSMR %in% smr_max, NutrientRegime %in% snr_max, BGC %in% subzones)# )
...
si_maxvals <- si_maxvals #%>% filter(DD5<750)#, DD5<1500)
...
ggp <- ggplot(si_maxvals, aes(DD5, SI50, label = PlotNumber)) +           
  geom_point()+
  geom_abline(intercept = intercept_bl, slope = slope_bl, color="red", 
               linetype="dashed", size=1.5)+
    geom_abline(intercept = intercept_pl, slope = slope_pl, color="blue", 
               linetype="dashed", size=1.5)+
    geom_abline(intercept = intercept_fd, slope = slope_fd, color="orange", 
               linetype="dashed", size=1.5)+
  xlim(0,2000)+
  ylim(0,50)+
  ggtitle("Climate Potential SI50 by DD5")+
