...
fit.bl <- lm(MeanSI ~ poly(SNR, aSMR, degree = 4, raw = TRUE), data = SIcomb)###Create polynomial model of degree 4 (create seperate model for each species)
...
SIFill <- foreach(Spp = c("Pl","Sx","Fd","Bl","Lw","Cw"), .combine = rbind) %do% {
  sibec <- read.xlsx("SIBEC_for_Portfolio2.xlsx", sheet = 1)
  sibec <- sibec[sibec$TreeSpp == Spp,]
